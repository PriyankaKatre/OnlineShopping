...
import { getLoggedInUserInfo, loginUser } from '../../../../redux/user/userThunk';
...
const UserLogin = () => {
    ...
    const { emailError, passwordError } = userErrorState
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const Resultaction = await dispatch(loginUser(userState) as any)
  ...
  return (
      <>
        <section className="px-5 md:px-10 lg:px-20">
            <p className="text-3xl p-3 bg-orange-200">Login Here</p>
        </section>
                     ...
                     <form onSubmit={handleSubmit}>
                        ...
                        <div className="pt-5">
                            <input
                                ...
                                type="password"
                                className={`block w-full rounded-md py-1.5 pl-7 pr-20
                                   text-gray-900 ring-1 ring-transparent focus:outline-none
                                   ${passwordError && 'ring-2 ring-red-400 ring-inset'}`}
                        ...
                        <div className="pt-5">
                              <input
                                ...
                                value="Login"
                                disabled={disabled}
                                className={`bg-gray-600 px-4 py-1 m-2 rounded text-white cursor-pointer
                                ${disabled && 'cursor-not-allowed'}`}
                            />
                        </div>
                        <span className="pt-5">New User?
                            <Link to="/users/login">
                                <strong> Login</strong>
                            </Link>
                        </span>
